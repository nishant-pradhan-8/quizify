...
export const getQuiz = async(req:Request, res:Response) =>{
    try {
      ...
      if(imageText.length===0){
        res.status(400).json({
          status: 'error',
          message: 'Unable to generate quiz. Make sure the image includes legible text and is of high resolution.',
          data: null
      });
      return;
      }
...
export const downloadQuiz = async (req: Request, res: Response) => {
  try {
    ...
    quiz.forEach((q: QuizQuestion, index: number) => {
      doc.fontSize(14).fillColor("black").text(`${index + 1}. ${q.question}`);
